...
public class PreviewAdapter extends XmlDomListAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (item instanceof XmlDom) {
			...
			try {
				String areaId = xmlDom.getElement().getAttribute(Constants.ID);
				view.setTag(areaId);
				if (Utils.isInFavorites(activity, areaId)) {
					((ImageView) view.findViewById(R.id.favorite))
							.setImageResource(android.R.drawable.star_big_on);
				}
				((ImageView) view.findViewById(R.id.favorite)).setTag(areaId);
